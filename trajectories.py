...
def trajectoriesProp(ldicc,traj):
    ...
    for kk in ldicc:
        ...
        for k in range(1,len(lldic)-1):
            ...
            if in_node not in lisTran.keys():
        ...
        for j in range(len(lldic)):
            ...
            if lldic[j] not in alisPos.keys():
...
# to plot     uncomment
    ...
    plt.xticks(pos)
    ylabes=[]
    for i in ved_trans[u_node].keys():
...
# to plot     uncomment
    ...
    # plt.show()
    return bot_di,outfile_name
def plot_transit_timeline(intervals,transit,tot_interv, ved_trans,npo,ndls,write_to_file=False):
    filedir='S_out_figs'
    try:
        os.stat(filedir)
    except:
